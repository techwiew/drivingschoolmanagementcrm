...
const ensureRoles = (req, res, roles) => {
...
const resolveTrainerProfile = async (trainerId) => {
...
router.get('/', async (req, res) => {
    try {
        const authUser = getAuthUser(req);
        let whereClause = { schoolId: authUser.schoolId };
...
router.get('/:id/students', async (req, res) => {
    try {
        ...
        if (authUser.role === 'STUDENT') {
            ...
            if (!studentProfile || schedule.studentId !== studentProfile.id)
                return res.json([]);
            const attendance = await prisma.attendance.findFirst({
